(function() {
  ...
  Worker = (function() {
    function Worker(config, workerType, nextWorkerType1, SYNC_ENDPOINT) {
      ...
      this.createHeartbeats = bind(this.createHeartbeats, this);
    ...
    Worker.prototype.createHeartbeats = function() {
      var heartbeatService;
      heartbeatService = this.app.service("heartbeat");
      return heartbeatService.create({
        type: this.workerType,
        uuid: this.workerUuid
      }, (function(_this) {
        return function(error, heartbeat) {
          if (!error) {
            console.log("Success creating heartbeat", heartbeat);
            return _this.workerHeartbeat.beginLife(heartbeat.type, heartbeat.uuid, function(error, heart) {
              if (!error) {
                return console.log("Created", heartbeat.type, heartbeat.uuid);
              }
            });
          } else {
